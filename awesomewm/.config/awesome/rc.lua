...
-- kbdcfg.add_primary_layout("Mine", beautiful.de_layout, "mine")
...
kbdcfg.widget:buttons(gears.table.join(
	...
	awful.button({}, 3, function() kbdcfg.switch_prev() end)
))
--]]
...
local inst_ram_widget = ram_widget()
local inst_cpu_widget = cpu_widget()
...
local globalkeys = gears.table.join(
	...
	end, { description = "Play/Pause current track", group = "Audio" }),
	...
	end, { description = "Stop current track", group = "Audio" }),
	...
	end, { description = "Mute/Unmute microphone", group = "Audio" }),
	-- screenshot
	...
	awful.key({ "Control", "Mod4", "Mod1" }, "space", function()
		...
		if handle then
			...
			naughty.notify("could not execute command...")
		end
	end, { description = "Switch the keyboard layout", group = "misc" })
)
